...
- **No project tracker** — this plugin diagnoses structure, it doesn't track tickets. If a diagnosis produces follow-up work (a reorg, a RACI rollout), that work belongs in whatever tracker the org already uses, outside this plugin's scope. **Linear** is pre-wired in `.mcp.json` if you want live issue context during a diagnosis.
