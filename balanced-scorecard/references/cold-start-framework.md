# Cold-start framework — claude-for-strategy

Every first-party plugin's `cold-start-interview` skill follows this framework. Plugin skills add only **plugin-specific** questions on top; org-wide facts live once in the shared org profile.

## Invocation

| Command | Behaviour |
|---|---|
| `/<plugin>:cold-start-interview` | Detect existing setup; offer quick vs full if mode not specified |
| `/<plugin>:cold-start-interview --quick` | Short path: org gaps (if any) + 3–5 plugin questions; sensible defaults elsewhere |
| `/<plugin>:cold-start-interview --full` | Full org interview (when needed) + full plugin interview; review seed documents |
| `/<plugin>:cold-start-interview --redo` | Ignore existing profiles for this run; re-interview and overwrite on confirmation |
| `/<plugin>:cold-start-interview --check-integrations` | Report MCP connector status for this plugin; no interview unless user asks to continue |
| `/<plugin>:cold-start-interview --resume` | Continue a paused interview from the saved session file |
...
| `~/.claude/plugins/config/claude-for-strategy/<plugin>/cold-start-resume.json` | Paused interview state (questions answered, remaining steps, draft field values) |
...
2. Run the **plugin-specific** questions defined in the skill (below the framework section in each `cold-start-interview/SKILL.md`).
...
**Pause:** If the user must stop mid-interview, write `cold-start-resume.json` with: `plugin`, `mode` (quick/full), `started_at`, `org_complete` (bool), `answers` (object of field → value), `remaining_steps` (array), `last_step_completed`. Tell the user to run `/<plugin>:cold-start-interview --resume`.
...
4. Mention `org-profile.md` is shared — other plugins will reuse it on their first cold-start.

## Living profile rules

- **`cold-start-interview`** is the only skill that may **auto-apply** a full profile write (after confirmation above).
