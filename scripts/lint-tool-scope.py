...
Runs over every `managed-agents/*/agent.yaml` and checks the orchestrator's
`tools:` block for the privilege-escalation gaps called out below — keeping
write and external-channel access on the leaves, not the orchestrator:
...
COOKBOOKS_DIR = ROOT / "managed-agents"
...
def _lint_one(path: Path) -> list[str]:
    ...
    for idx, entry in enumerate(tools):
        ...
        if ttype == "mcp_toolset":
            name = entry.get("mcp_server_name", "?")
            errs.append(
