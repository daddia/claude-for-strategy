...
This is a **cookbook, not a product.** It assumes programme artefacts live in the workspace (markdown RAID, milestone exports, decision log). Teams that keep RAID and milestones in Jira or Linear should wire a read connector on the reader tier or export to local files before the run.
...
- **Point at your RAID and milestone sources.** The default reader tier reads local files only. If RAID lives in Jira via Atlassian MCP or Linear via Linear MCP, add an `mcp_toolset` for `atlassian` or `linear` on `reader.yaml` only — never on compute or write-holder.
