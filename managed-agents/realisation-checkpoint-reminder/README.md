...
This is a **cookbook, not a product.** It assumes the benefits register and tracking log live in the workspace (markdown, spreadsheet export, or files under `value-realisation/data/`). Teams that keep registers in Confluence, Jira, or Linear should wire a read connector on the reader tier or export to a local file before the run.
...
- **Point at your register source.** The default reader tier reads local files only. If the register lives in Confluence via Atlassian MCP or in Linear via Linear MCP, add an `mcp_toolset` for `atlassian` or `linear` on `reader.yaml` only — never on compute or write-holder.
